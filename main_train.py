...
def train(model, device, train_loader, optimizer, epoch,time_begin):
    model.train()
...
def model_test(model, device, test_loader,time_epoch0):
    global  best_acc
    model.eval()
    test_loss = 0
    correct = 0
    ...
    current_best_acc=correct / len(test_loader.dataset)
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train(model, DEVICE, train_loader, optimizer, epoch,time_begin)
        scheduler.step()
        model_test(model, DEVICE, test_loader,time_epoch0)
    print("best acc:{}".format(best_acc))
    #model.save('checkpoints/SGL-NN_1000_1.pth')
