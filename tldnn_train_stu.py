...
from data import RML2016b,ESDataset,ESDatasetX
torch.__version__
...
LEARN_RATE = 0.005
MODEL1_PATH = 'checkpoints/4_best_model1_x.pth'
MODEL2_PATH = 'checkpoints/4_best_model1_x.pth'
...
Is_train_conf_model = 1  # 0 for use trained main_model form MODEL2_PATH, 1 for trained conf_model from scratch
Bh = 0.75
Bl = 0.20
...
def train(model, device, train_loader, optimizer, epoch, reg_loss,time_begin):
    model.train()
...
def do_test(model, device, test_loader,time_epoch0):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def converged_net(data):
    ...
    for i in range(0, predict_fine.size(0) - 1):
        if predict_main[i] in conf_num:
            ...
            predict_fine[i]=predict_conf[i]
    return predict_fine


# Step1: train the main network
print("\nStep1: train the main network")
train_dataset = ESDatasetX(train=True)
train_loader = DataLoader(train_dataset,
                          batch_size=BATCH_SIZE,
                          shuffle=True,
                          num_workers=2)
test_dataset = ESDatasetX(train=False)
...
def main():
    ...
    if Is_train_main_model:
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            train(model_main, DEVICE, train_loader, optimizer_main, epoch, reg_loss_main,time_begin)
            scheduler_main.step()
            do_test(model_main, DEVICE, test_loader,time_epoch0)
    else:
        print("use the trained network as the main network")

    model_main.save('checkpoints/model1.pth')
    ...
    conf_matrix = confusion_matrix(model_main, conf_matrix,converge=False)
    confusion_num = []
    confusion_class = []
    ...
    if Is_train_conf_model:
        fine_train_dataset = ESDatasetX(train=True, fine_train=True, conf_class=conf_class)
        fine_train_loader = DataLoader(fine_train_dataset,
                                       batch_size=BATCH_SIZE,
                                       shuffle=True,
                                       num_workers=2)
        fine_test_dataset = ESDatasetX(train=False, fine_train=True, conf_class=conf_class)
        ...
        for epoch in range(1, EPOCHS + 1):
            time_epoch0 = time.time()
            train(model_conf, DEVICE, fine_train_loader, optimizer_conf, epoch, reg_loss_conf,time_begin)
            scheduler_conf.step()
            do_test(model_conf, DEVICE, fine_test_loader,time_epoch0)
    else:
        print("use the trained network as the confuse network")

    model_conf.save('checkpoints/model2.pth')
